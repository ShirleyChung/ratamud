# 事件系統改用獨立執行緒 - 實現說明

## 📋 改進目標

將事件檢查邏輯從主遊戲循環移到獨立執行緒中，提升性能和架構清晰度。

## 🔧 實現內容

### 1. 新增檔案

**src/event_check_thread.rs**
- 創建 `EventCheckThread` 結構
- 類似於 `NpcAiThread` 的設計
- 在獨立線程中定期檢查事件觸發條件
- 返回觸發的事件 ID 列表和日誌

### 2. 修改的檔案

#### src/lib.rs
- 添加 `pub mod event_check_thread;`

#### src/main.rs
- 添加 `mod event_check_thread;`

#### src/world.rs
- 添加字段：`pub event_check_thread: Option<EventCheckThread>`
- 添加方法：
  - `get_event_logs()` - 獲取事件檢查日誌
  - `get_triggered_events()` - 獲取觸發的事件 ID
- 在 `Drop` trait 中停止事件檢查線程

#### src/app.rs
- 在 `run_main_loop()` 啟動事件檢查執行緒
- 刪除舊的事件檢查邏輯（`check_and_execute_events` 函數）
- 在主循環中：
  - 同步時間到事件檢查執行緒
  - 獲取觸發的事件
  - 在主執行緒中執行事件動作

## 🎯 架構設計

### 線程職責分離

```
┌─────────────────┐
│   主執行緒      │
│  (Main Loop)    │
│                 │
│ • UI 渲染       │
│ • 玩家輸入      │
│ • 事件執行      │ ←── 接收觸發的事件 ID
└────────┬────────┘
         │
    同步時間 ↓
         │
┌────────┴────────┐
│ 事件檢查執行緒  │ 
│ (1 秒檢查一次)  │
│                 │
│ • 檢查時間觸發  │ 
│ • 檢查 cron     │
│ • 返回事件 ID   │ ──→ 回傳到主執行緒
└─────────────────┘
```

### 工作流程

1. **啟動階段**
   ```rust
   // 創建事件數據副本
   let events_data = Arc::new(Mutex::new(...));
   let time_for_event = Arc::new(Mutex::new(...));
   
   // 啟動執行緒（每 1 秒檢查一次）
   let event_check_thread = EventCheckThread::new(check_fn, 1000);
   ```

2. **執行階段（每 1 秒）**
   ```
   事件檢查執行緒：
   ├─ 獲取當前時間
   ├─ 檢查是否同一分鐘（避免重複）
   ├─ 遍歷所有事件
   │  └─ 檢查時間觸發器
   │     ├─ 檢查時間範圍
   │     ├─ 檢查 cron 表達式
   │     └─ 記錄觸發的事件 ID
   └─ 返回 (日誌列表, 事件 ID 列表)
   ```

3. **同步階段（主循環）**
   ```rust
   // 更新時間
   game_world.update_time();
   
   // 同步到事件執行緒
   *time_lock = game_world.time.clone();
   
   // 獲取觸發的事件
   let triggered_events = game_world.get_triggered_events();
   
   // 執行事件
   for event_id in triggered_events {
       execute_event(&event, game_world, me, output_manager);
   }
   ```

## ✨ 優勢

### 1. 性能提升
- **減少主循環負擔**：事件檢查不再阻塞主遊戲循環
- **並行處理**：事件檢查與遊戲邏輯並行執行
- **定時檢查**：只在需要時檢查（1 秒一次），不是每幀都檢查

### 2. 架構改善
- **職責分離**：檢查邏輯和執行邏輯分開
- **線程安全**：使用 `Arc<Mutex>` 安全共享數據
- **易於擴展**：可以輕鬆添加更多觸發器類型

### 3. 一致性
- **統一設計**：與 `NpcAiThread`、`TimeThread` 相同的模式
- **可維護性**：清晰的線程管理和生命週期

## 🔍 當前實現範圍

### 支持的觸發器
目前在獨立執行緒中只支持：
- ✅ **時間觸發器** (`TriggerType::Time`)
  - cron 表達式（分鐘和小時）
  - 時間範圍限制

### 未來擴展
可以添加更多觸發器類型：
- ⏳ 隨機事件 (`TriggerType::Random`)
- ⏳ 位置觸發 (`TriggerType::Location`)
- ⏳ 條件觸發 (`TriggerType::Condition`)

## 📊 性能對比

### 之前（主循環檢查）
```
主循環（60 FPS）
├─ 每幀檢查事件 ❌
├─ 遍歷所有事件
├─ 檢查觸發條件
└─ 執行事件
```
**問題**：每秒檢查 60 次，但大部分時候沒有事件觸發

### 現在（獨立執行緒）
```
主循環（60 FPS）          事件執行緒（1 Hz）
├─ 渲染 UI              ├─ 每秒檢查一次 ✅
├─ 處理輸入             ├─ 遍歷所有事件
├─ 獲取觸發事件 ✅       ├─ 檢查觸發條件
└─ 執行事件            └─ 返回事件 ID
```
**優勢**：主循環更輕量，事件檢查獨立進行

## 🛠️ 技術細節

### 線程安全
```rust
// 使用 Arc<Mutex> 共享數據
let time_for_event = Arc::new(Mutex::new(game_world.time.clone()));
let events_data = Arc::new(Mutex::new(...));
```

### 避免重複檢查
```rust
// 靜態變量追蹤上次檢查時間
static LAST_CHECK: Mutex<(u64, u8, u8)> = Mutex::new((0, 0, 0));
{
    let mut last = LAST_CHECK.lock().unwrap();
    if (*last) == (current_day, current_hour, current_minute) {
        return (logs, triggered_event_ids);
    }
    *last = (current_day, current_hour, current_minute);
}
```

### 優雅關閉
```rust
impl Drop for GameWorld {
    fn drop(&mut self) {
        if let Some(ref mut thread) = self.event_check_thread {
            thread.stop();  // 自動停止執行緒
        }
    }
}
```

## 📝 測試建議

### 1. 基本功能測試
```bash
cargo run --release --bin main
# 觀察事件日誌是否正常顯示
```

### 2. 性能測試
- 觀察 CPU 使用率
- 檢查 UI 流暢度
- 確認事件觸發準確性

### 3. 壓力測試
- 添加大量事件
- 長時間運行
- 檢查內存洩漏

## 🎉 總結

事件系統改為獨立執行緒後：
- ✅ 主循環更輕量
- ✅ 事件檢查不阻塞遊戲
- ✅ 架構更清晰
- ✅ 易於維護和擴展
- ✅ 與其他執行緒模式一致

---

**實現時間**: 2025-12-16  
**狀態**: ✅ 已完成並測試
