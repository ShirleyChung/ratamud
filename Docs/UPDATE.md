...
13. [資料相容性說明](#資料相容性說明)
14. [Minimap 網格更新](#minimap-網格更新)
15. [Minimap 對齊調整](#minimap-對齊調整)
16. [Minimap 40x10 更新](#minimap-40x10-更新)
17. [所有指令後更新 Minimap](#所有指令後更新-minimap)
...
新增訊息回捲功能，使用 **PageUp** 和 **PageDown** 鍵（或 **Shift + ↑/↓**）可以查看歷史訊息。

## 使用方法

### 鍵盤操作
- **PageUp** - 向上捲動訊息（查看更早的訊息）
- **PageDown** - 向下捲動訊息（查看更新的訊息）
- **Shift + ↑** - 向上捲動訊息（替代方案，部分終端可能不支援）
- **Shift + ↓** - 向下捲動訊息（替代方案，部分終端可能不支援）
...
### ✅ 優先順序清晰
1. **PageUp/PageDown 或 Shift + 方向鍵** → 訊息捲動
2. **方向鍵（地圖開啟時）** → 移動地圖視圖
3. **方向鍵（正常模式）** → 移動角色
...
### 1. PageUp/PageDown（最高優先級）
```
任何時候按 PageUp 或 PageDown 都會捲動訊息
（Shift+↑/↓ 在支援的終端也可以使用）
```
...
# Shift+方向鍵仍然捲動訊息（如果終端支援）
PageUp/PageDown - 捲動訊息
```
...
1. **PageUp/PageDown 支持**
   ```rust
   KeyCode::PageUp => {
       output_manager.scroll_up_page(20);
   }
   KeyCode::PageDown => {
       output_manager.scroll_down_page(20);
   }
   ```
...
2. **Home/End 支持**
   ```rust
   KeyCode::Home => {
       output_manager.scroll_to_top();
   }
   KeyCode::End => {
       output_manager.scroll_to_bottom();
   }
   ```
...
**PageUp/PageDown** 是主要的捲動方式，幾乎所有終端都支援。

**Shift+方向鍵** 作為替代方案，但某些終端模擬器可能：
- 攔截 Shift+方向鍵用於文字選擇
- 不傳遞 Shift 修飾鍵給應用程式
- 需要特殊設定才能使用

**建議**：優先使用 PageUp/PageDown 鍵進行訊息捲動。

## 修復記錄

### 問題 1：捲動無效（初次修復）
**原因**：`print` 函數將 scroll 設為 `messages.len() - 1`，當訊息很多時這個值遠大於 `max_scroll`，導致 `scroll_up()` 減 1 後仍然超過最大值。

**初步修復**：將 scroll 設為 `usize::MAX`
```rust
...
### 問題 2：捲動仍然無效（第二次修復）
**原因**：`usize::MAX` 太大，`scroll_up()` 減 1 後還是 `usize::MAX - 1`，仍然超過 `max_scroll`！

**最終修復**：在 `scroll_up()` 和 `scroll_down()` 中先正規化 scroll 值
```rust
pub fn scroll_up(&mut self) {
    // 先確保 scroll 不會超過合理範圍
...
**效果**：
1. 新訊息時，scroll = usize::MAX（表示「最新」）
2. 按 PageUp 時，先正規化為 messages.len() - 1，然後減 1
3. 現在可以正確向上/向下捲動了！

### 問題 3：捲動效果不明顯（第三次修復）
**原因**：每次只捲動 1 行，但由於 `render_output` 中的 `scroll.min(max_scroll)` 限制，需要捲動很多次才能看到效果。

**範例**：
- 30 條訊息，畫面高度 20
- max_scroll = 10
- scroll = 29 時，顯示 messages[10..30]
- scroll_up() 後 scroll = 28，但 min(28, 10) = 10，還是顯示 messages[10..30]
- 需要減到 scroll = 9 才會看到變化！
...
**最終效果**：按一次 PageUp/PageDown 就能明顯看到訊息捲動！
...
**第628-632行：網格大小**
```rust
let grid_width = 30;   // 從 15 改為 30
let grid_height = 10;  // 保持不變
let half_width = grid_width / 2;   // 15
let half_height = grid_height / 2; // 5
```
...
- **第310行**: `get_minimap()` 方法
  - 從 `Alignment::Center` 改為 `Alignment::Left`
  
- **第375行**: `render_minimap()` 方法
  - 從 `Alignment::Center` 改為 `Alignment::Left`

### 2. 調整 minimap 寬度以 fit 內容
**文件**: `src/app.rs`

- **第82-83行**: 調整寬度和高度
  - 寬度改為固定 `42u16`（40字符網格 + 邊框2）
  - 高度為 `18u16`
...
移動後會更新minimap，但執行其他指令（如`conq u`）後，minimap不會更新，看不到地圖變化。

## 解決方案

在 `handle_command_result` 函數的最後，統一檢查並更新minimap。
...
**移除重複更新**：從 `handle_movement` 函數中移除重複的minimap更新邏輯（原第751-754行）。

### 效果

現在所有命令執行後都會自動更新minimap（如果minimap已打開），包括：
- `conq u` (征服方向)
- `get` / `drop` (拾取/丟棄物品)
- `create` / `destroy` (創建/刪除NPC或物品)
- `summon` (召喚NPC)
- `flyto` (傳送)
- 所有其他命令
